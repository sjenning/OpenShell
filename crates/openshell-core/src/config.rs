...
impl FromStr for ComputeDriverKind {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "auto" => Ok(Self::Auto),
            other => Err(format!(
                "unsupported compute driver '{other}'. expected one of: kubernetes, vm, docker, podman, auto"
            )),
        }
    }
}
...
mod tests {
    ...
    fn compute_driver_kind_parses_supported_values() {
        ...
        assert_eq!(
            "auto".parse::<ComputeDriverKind>().unwrap(),
            ComputeDriverKind::Auto
        );
    }
    ...
    #[test]
    fn config_defaults_to_empty_drivers() {
        assert_eq!(Config::new(None).compute_drivers, vec![]);
    }
