...
async fn build_compute_runtime(
    ...
    match driver {
        ...
        ComputeDriverKind::Auto => unreachable!(
            "Auto should never reach build_compute_runtime; \
            it is resolved by configured_compute_driver"
        ),
    }
}
    ...
    match config.compute_drivers.as_slice() {
        [] => openshell_core::config::detect_driver().ok_or_else(|| {
            Error::config(
                ...
                set --drivers or OPENSHELL_DRIVERS to auto, kubernetes, podman, docker, or vm",
            )
        }),
        [
